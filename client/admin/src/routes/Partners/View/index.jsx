...
class Dynamic extends React.Component {
  ...
  delete = (id) => {
    axios.delete(`/api/v2/partners/delete/${id}`).then((res) => {
      const {
        data: { message },
        statusText,
      } = res;
    ...
    componentWillMount = () => {
      axios.get('/api/v2/partners/getAll').then((result) => {
        const { data } = result;
        const columns = [
          {
            title: 'name',
        ...
        data.map((element) => {
          element.action = (
            <span>
              <Link to={`/admin/partners/${element.id}`} className="icon icon-feedback" />
          ...
          element.title = (
            <Link to={`/admin/partners/${element.id}`}>{element.title}</Link>
          );
          return element;
        });
        this.setState(() => ({ data }));
