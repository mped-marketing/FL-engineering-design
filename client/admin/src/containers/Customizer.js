...
import {
  Button, Drawer, Form, message, Radio,
} from 'antd';
...
import { 
onLayoutTypeChange, onNavStyleChange, setThemeColorSelection, setThemeType
...
class Customizer extends Component {
  ...
  getColorPicker = varName => (
    <div key={varName} className="gx-media gx-mb-1">
      <div className="gx-ml-1 gx-mr-4">
        ...
        >
          <span
            className="gx-pointer gx-text-capitalize gx-media-body"
          >
  ...
  getCustomizerContent = () => {
    ...
    const {
      themeType, layoutType, navStyle, colorSelection,
    } = this.props;
...
const mapStateToProps = ({ settings }) => {
  const {
    themeType, width, colorSelection, navStyle, layoutType,
  } = settings;
  return {
    themeType, width, colorSelection, navStyle, layoutType,
  };
